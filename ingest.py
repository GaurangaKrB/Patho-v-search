...
def load_patch_pkl(path: str) -> Tuple[str, np.ndarray, Optional[np.ndarray], Dict[str, Any]]:
    """
    Returns (slide_id, patch_vecs[N,1536], None, payload_template)
    for .pkl files that contain a list of lists (patch embeddings).

    Expected pickle format: list[list[float]] of shape (N, 1536).
    """
    slide_id = infer_slide_id(path)
    payload_template: Dict[str, Any] = {
        "source_file": os.path.basename(path),
    ...
    patch_vecs = np.asarray(obj, dtype=np.float32)

    if patch_vecs.ndim != 2:
        raise ValueError(
            f"Expected 2D array from pickle, got shape={patch_vecs.shape} in {path}"
        )

    if patch_vecs.shape[1] != DIM:
        raise ValueError(
            f"Patch dim mismatch in {path}: got {patch_vecs.shape[1]}, expected {DIM}"
        )

    return slide_id, patch_vecs, None, payload_template
...
def ingest_patches(
    ...
    files = list_files_with_extensions(patches_dir, (".pt", ".h5", ".pkl"))
    if limit_files:
        files = files[:limit_files]

    if not files:
        print(f"No patch .pt/.h5/.pkl files found in {patches_dir}")
        return 0, 0
    ...
    for path in tqdm(files, desc="Patch files"):
        if path.endswith(".h5"):
            ...
            slide_id, patch_vecs, coords, payload_template = load_patch_pkl(path)
        else:
